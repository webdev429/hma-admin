                ...
                @can('create', App\Make::class)
                  <div class="row">
                    <div class="col-12 text-right">
                      <a href="{{ route('truckmake.create') }}" class="btn btn-sm btn-rose">{{ __('Add') }}</a>
                    </div>
                  </div>
                @endcan
                <div class="table-responsive">
                  <table id="datatables" class="table table-striped table-no-bordered table-hover" style="display:none">
                    <thead class="text-primary">
                      ...
                      @can('manage-truckmakes', App\User::class)
                        <th class="text-right">
                          {{ __('Actions') }}
                        </th>
                      @endcan
                          ...
                          @can('manage-truckmakes', App\User::class)
                            @if (auth()->user()->can('update', $item) || auth()->user()->can('delete', $item))
                              <td class="td-actions text-right">
                                <form action="{{ route('truckmake.destroy', $item) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    
                                    @can('update', $item)
                                      <a rel="tooltip" class="btn btn-success btn-link" href="{{ route('truckmake.edit', $item) }}" data-original-title="" title="">
