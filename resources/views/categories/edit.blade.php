                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                                        <textarea cols="30" rows="10"
                                            ...
                                            aria-required="true">{{ old('description', $category->description) }}</textarea>
                                    ...
                                    <div class="form-group{{ $errors->has('email') ? ' has-danger' : '' }}">
                                        <select class="selectpicker col-sm-12 pl-0 pr-0" name="specifics[]"
                                            ...
                                            @foreach ($specifics as $specific)
                                                <option value="{{ $specific->id }}" {{ in_array($specific->id, old('specifics', $category->specifics->pluck('id')->toArray()) ?? []) ? 'selected' : '' }}>{{ $specific->name }}</option>
                                            @endforeach
                                        </select>
